...
public class Processor {
	 ...
	 * If running locally, predefined topic configurations specified in the
	 * constants above will be used.<br>
	 * If running on any cloud environment,the topic configurations will be obtained
	 * from UPS.
	 */
	private void createAndExecuteConsumers() {
		...
		if (applicationProperties.getApplicationEnvCode().equals(LOCAL_ENV)) {
			topicConfigurations.put(KafkaConfig.LOCAL_CUSTOMER_EVENT_TOPIC,
					KafkaConfig.LOCAL_CUSTOMER_EVENT_TOPIC_CONFIGURATION);
			topicConfigurations.put(KafkaConfig.LOCAL_ACCOUNT_EVENT_TOPIC,
					KafkaConfig.LOCAL_ACCOUNT_EVENT_TOPIC_CONFIGURATION);
			topicConfigurations.put(KafkaConfig.LOCAL_TEST_TOPIC,
					KafkaConfig.LOCAL_TEST_TOPIC_CONFIGURATION);
			topicConfigurations.put(KafkaConfig.LOCAL_TEST_TOPIC_2,
					KafkaConfig.LOCAL_TEST_TOPIC_2_CONFIGURATION);
		}
		...
		for (Map.Entry<String, String> entry : topicConfigurations.entrySet()) {
			...
			registerTopics(topicConfiguration.getTopic(), topicConfiguration.getConsumerGroup(),
					topicConfiguration.getEnvironment(), 0, EMPTY, INITIAL_DATE);
	...
	/**
	 * All topics (with initial settings) are registered/set against the local
	 * topics cache for future reference.
	 * 
	 * @param topic                 topic
	 ...
	 */
	private void registerTopics(String topic, String consumerGroup, String environment, long offset, String payload,
			String latestPayloadDateTime) {
		TopicDetail topicDetail = constructTopicDetail(topic, consumerGroup, environment, offset, payload,
				latestPayloadDateTime);
...
//                        logger.info("Constructing topic detail for record at offset: " + consumerRecord.offset());
						TopicDetail topicDetail = constructTopicDetail(topicConfiguration.getTopic(),
								topicConfiguration.getConsumerGroup(), topicConfiguration.getEnvironment(),
								consumerRecord.offset(), payload, sdf.format(timestamp));
...
//                                        + " the record is: " + consumerRecord.value());
	...
	/**
	 * Checks for fetch-record requests from the client and returns the record value
	 * detail if it finds it based on offset.
	 * 
			...
			if (recordRequest.getTopicName().equals(topicConfiguration.getTopic())) {
				...
				try {
					if (customerRecords.iterator().hasNext()) {
						...
						logger.info("**** record fetched for topic: " + topicConfiguration.getTopic() + " is: " + record
								+ " at offset: " + consumerRecord.offset() + " inserted at time: "
	...
	private void fetchLatestRecord(KafkaConsumer<String, ?> consumer, TopicConfiguration topicConfiguration) {
		...
		try {
			if (customerRecords.iterator().hasNext()) {
				...
				record = consumerRecord.value().toString();
				logger.info("**** record fetched for topic: " + topicConfiguration.getTopic() + " is: " + record
						+ " at offset: " + consumerRecord.offset() + " inserted at time: " + consumerRecord.timestamp()
						+ " ****");
			} else {
				record = OUT_OF_RANGE_MSG;
			}
		...
		if (consumerRecord != null) {
			topicDetail = constructTopicDetail(topicConfiguration.getTopic(), topicConfiguration.getConsumerGroup(),
					topicConfiguration.getEnvironment(), consumerRecord.offset(), record,
					sdf.format(consumerRecord.timestamp()));
			updateTopic(topicDetail);
		}
		consumer.seekToEnd(topicPartitions);
	}
	...
	/**
	 * Updates the specified topic (contained within the topic detail) in the local
	 * topic cache with the latest response detail.
	 * 
	 ...
	 * @return topicDetail
	 */
	private TopicDetail constructTopicDetail(String topic, String consumerGroup, String environment, long offset,
			String payload, String latestPayloadDateTime) {
	...
	/*
	 * private void resetConsumers() { try { shutdownConsumers();
	 * Thread.sleep(4000); topics.clear(); recordRequests.clear(); startConsumers();
	 * } catch (InterruptedException e) { logger.error("Reseting consumers: " +
	 * e.getMessage()); } }
	 */
